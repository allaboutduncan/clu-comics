...
DAILY_RATE_LIMIT_THRESHOLD = 60


class DailyRateLimitError(ProviderRateLimitError):
    """Raised when the Metron daily rate limit has been reached."""

    def __init__(self, retry_after):
        super().__init__(retry_after, f"Daily limit reached. Retry in {retry_after}s")


def handle_metron_rate_limit(func):
    @wraps(func)
    def wrapper(*args, **kwargs):
        try:
            ...
            if retry_after > DAILY_RATE_LIMIT_THRESHOLD:
                app_logger.error(
                    f"Metron daily limit reached: {retry_after}s remaining."
                )
                raise DailyRateLimitError(retry_after)

            app_logger.info(f"Metron rate limit hit. Waiting {retry_after}s...")
...
class MetronProvider(BaseProvider):
    ...
    def test_connection(self) -> bool:
        ...
        except (RateLimitError, DailyRateLimitError):
            raise
        except Exception as e:
